...
def train():
    for epoch in range(config['epochs']):
        ...
        if (epoch + 1) % 2 == 0:
            ...
            print("train f1: ", train_f1_ls[-1], "test f1:", valid_f1_ls[-1], end=" ")

        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(),
                       config['save_dir'] + "_".join(time.ctime()[4:-5].replace(":", "_").split(" ")) + str(
                           epoch) + ".model")
        print("valid:", _valid_cal_f1("", model))
...
if __name__ == '__main__':
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(
        config['epochs'] * train_set_l / config['batch_size'] / 20), gamma=0.9)
